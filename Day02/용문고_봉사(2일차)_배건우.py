...
class PongEnv:
    ...
    def reset(self):
        ...
        if self.score == 0:
            ...
            self.ball_dx = np.random.choice([-0.01*self.score, 0.01*self.score])  # 좌우 랜덤
            self.ball_dy = 0.01*self.score  # 아래로 이동
    ...
    def render(self):
        ...
        import pygame
        color = self.rb[self.score % len(self.rb)]
        bc =self.rb[self.bk % len(self.rb)]
        # 배경
        ...
        # Miss 횟수 표시
        # sself.screen.blit(miss_text, (self.width - 150, 20))
        self.bk+=1
        pygame.display.flip()
